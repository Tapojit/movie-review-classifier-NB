...
class NaiveBayes:
    ...
    def update_model(self, bow, label):
        ...
        """
        def key_adder_2(key, token_map):
            self.class_word_counts[label][key]+=token_map[key]
        
        map(lambda k: key_adder_2(k, bow), bow)
    ...
    def log_likelihood(self, bow, label, alpha):
        ...
        all_log_probs=list(map(lambda k: math.log10(self.p_word_given_label_and_pseudocount(k, label,alpha)), bow.keys()))
        return sum(all_log_probs)
    ...
    def log_prior(self, label):
        ...
        """
        return math.log10(self.class_total_doc_counts[label]/sum(self.class_total_doc_counts.values()))
    ...
    def classify(self, bow, alpha):
        ...
        bow - a bag of words (i.e., a tokenized document)
        """
        log_poste_pos=self.unnormalized_log_posterior(bow, POS_LABEL, alpha)
        log_poste_neg=self.unnormalized_log_posterior(bow, NEG_LABEL, alpha)
        
        if log_poste_pos>log_poste_neg:
            return POS_LABEL
        else:
            return NEG_LABEL
    ...
    def wrong_classified_review(self, alpha):
        """
        Returns first wrong classified review
        """
        wrong_review=""
        actual_label=""
